...
def compute_ivt_metrics(
    ...
    calibration_value: str = "Eyetracker Calibration",
    ...
    if exclude_calibration and stimulus_col in df.columns:
        before = len(df)
        df = df[df[stimulus_col] != calibration_value]
        filtered = before - len(df)
        if filtered > 0:
            print(f"[Eval] Excluded {filtered} samples with stimulus '{calibration_value}'")
    elif exclude_calibration:
        print(f"[Eval] Warning: stimulus column '{stimulus_col}' not found; no calibration samples excluded")
