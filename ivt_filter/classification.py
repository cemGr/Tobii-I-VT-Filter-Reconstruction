...
from .velocity_calculation import VelocityCalculationStrategy, Ray3DAngle, Olsen2DApproximation
from .coordinate_rounding import NoRounding
...
class IVTClassifier:
    ...
    def _classify_sample(self, row: pd.Series) -> str:
        ...
        threshold = self.cfg.velocity_threshold_deg_per_sec
        if "window_any_invalid" in row and bool(row.get("window_any_invalid", False)):
        ...
        if velocity_final < fixation_cut:
            return "Fixation"
        # In-between band: stick with previous label if available
        prev_idx = row.name - 1
        if prev_idx >= 0 and "ivt_sample_type" in row.index:
            return row["ivt_sample_type"]
        return "Fixation"
